...
export async function startRecovery(applicationId: string, since: Date, endpointId?: string) {
  return db.$transaction(async tx => {
    await tx.$queryRaw`SELECT id FROM "Application" WHERE id = ${applicationId} FOR UPDATE`;
    if (endpointId && !await tx.endpoint.findFirst({ where: { id: endpointId, applicationId } })) throw new WorkspaceError(404, "Endpoint not found");
    if (await tx.recoveryJob.findFirst({ where: { applicationId, status: "PENDING" } })) throw new WorkspaceError(409, "A recovery is already running for this application");
    const app = await tx.application.findUniqueOrThrow({ where: { id: applicationId } });
    if (app.recoveryAvailableAt && app.recoveryAvailableAt > new Date()) throw new WorkspaceError(429, "Recovery rate limit: wait one minute");
    await tx.application.update({ where: { id: applicationId }, data: { recoveryAvailableAt: new Date(Date.now() + 60000) } });
    return tx.recoveryJob.create({ data: { applicationId, endpointId, since } });
  });
}
export async function drainRecovery() {
  ...
  for (const job of jobs) await db.$transaction(async tx => {
    ...
    const failed = await tx.$queryRaw<{ eventId: string; endpointId: string }[]>(Prisma.sql`
      ...
      WHERE e."applicationId" = ${job.applicationId} AND e."createdAt" >= ${job.since} AND e."createdAt" <= ${job.until}
