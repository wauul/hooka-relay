...
export function eventRateLimit() {
  ...
  if (!Number.isSafeInteger(value) || value < 1)
    throw new Error("Invalid EVENTS_RATE_LIMIT_PER_MINUTE");
  return value;
}
...
export async function admitEvent(applicationId: string) {
  ...
  return db.$transaction(async (tx) => {
    ...
    await tx.eventAdmission.deleteMany({
      where: { applicationId, createdAt: { lte: cutoff } },
    });
    const count = await tx.eventAdmission.count({ where: { applicationId } });
    if (count >= limit) {
      const first = await tx.eventAdmission.findFirstOrThrow({
        where: { applicationId },
        orderBy: { createdAt: "asc" },
      });
      return Math.max(
        1,
        Math.ceil((first.createdAt.getTime() + 60_000 - now.getTime()) / 1000),
      );
    }
