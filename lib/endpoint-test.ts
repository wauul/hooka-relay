...
export async function sendEndpointTest(endpoint: { id: string; applicationId: string; status: string; circuitState: string }) {
  ...
  const admitted = redisConfigured() ? await admitFixedWindow(key, 60, 5) : (await db.$queryRaw<{ hits: number }[]>`
    ...
    WHERE "IpRateBucket".hits < 5 RETURNING hits`).length > 0;
  if (!admitted) return { limited: 60 } as const;
