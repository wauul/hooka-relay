...
provider.register();
afterAll(async () => { await provider.shutdown(); context.disable(); trace.disable(); await db.$disconnect(); });
it("persists initial ingestion context and never overwrites it for an idempotent retry", async () => {
  ...
  try {
    ...
    expect(first.traceparent).toMatch(new RegExp("^00-" + "1".repeat(32) + "-[a-f0-9]{16}-01$"));
