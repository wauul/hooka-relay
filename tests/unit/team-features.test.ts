...
describe("role/action matrix", () => {
  ...
  for (const role of ["OWNER", "ADMIN", "MEMBER"] as Role[]) {
    for (const action of actions.filter(a => a !== "kick" && a !== "roles")) it(`${role} / ${action}`, () => expect(permitted(role, action)).toBe(allowed[role].includes(action)));
    for (const target of ["OWNER", "ADMIN", "MEMBER"] as Role[]) {
      it(`${role} kick ${target}`, () => expect(permitted(role, "kick", target)).toBe((role === "OWNER" && target !== "OWNER") || (role === "ADMIN" && target === "MEMBER")));
      it(`${role} change ${target}`, () => expect(permitted(role, "roles", target)).toBe(role !== "MEMBER" && target !== "OWNER"));
    }
  }
});
it("accepts the old key strictly within its grace window", () => {
  const now = new Date("2026-09-16T12:00:00Z");
  const app = { currentApiKey: "new", previousApiKey: "old", previousApiKeyExpiresAt: now };
  ...
  expect(keyIsValid({ ...app, previousApiKeyExpiresAt: null }, "old", now)).toBe(false);
});
it("validates grace configuration", () => {
  vi.stubEnv("API_KEY_GRACE_HOURS", "2"); expect(keyGraceHours()).toBe(2);
  vi.stubEnv("API_KEY_GRACE_HOURS", "0"); expect(() => keyGraceHours()).toThrow();
  vi.unstubAllEnvs();
});
it.each(["ACTIVE", "PAUSED"] as const)("pauses and resumes %s idempotently", state => {
