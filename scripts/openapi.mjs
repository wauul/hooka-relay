...
const schemas = {
  EventInput: object({ type: { ...string, minLength: 1, maxLength: 120, pattern: "^[A-Za-z0-9_.:-]+$" }, payload: {}, idempotencyKey: { ...string, minLength: 1, maxLength: 200, pattern: "^[!-~]+$" } }, ["type", "payload"]),
  EventResponse: object({ id: string, applicationId: string, type: string, payload: {}, idempotencyKey: string, operational: { type: "boolean" }, createdAt: date }),
  ...
  Endpoint: object({ id: string, url: string, eventTypes: array(string), circuitState: { enum: ["CLOSED", "OPEN", "HALF_OPEN"], type: "string" }, status: { enum: ["ACTIVE", "PAUSED", "DISABLED"], type: "string" }, userStatus: { enum: ["ACTIVE", "PAUSED"], type: "string" }, environment: string, kind: { enum: ["BUSINESS", "OPERATIONAL"], type: "string" }, deliveryRatePerMinute: nullable(integer), signatureFormat: { enum: ["LEGACY", "STANDARD"], type: "string" }, createdAt: date, successRate: nullable(integer) }),
  ...
  EndpointCreate: { allOf: [ref("EndpointConfiguration"), object({ url: { ...string, format: "uri", maxLength: 2000, description: "Public HTTPS on port 443; every resolved address must be public." }, eventTypes: { ...array({ ...string, maxLength: 120 }), minItems: 1, maxItems: 50, default: ["*"] } }, ["url"])] },
...
add("/api/v1/endpoints/{id}/signature-format", "patch", "Choose signature format", object({ signatureFormat: string }), { body: object({ signatureFormat: { type: "string", enum: ["STANDARD", "LEGACY"] } }), description: "Existing unscoped key required. Explicit opt-in for existing legacy receivers; deploy verification support before switching." });
