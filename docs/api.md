...
Dashboard session routes: `GET/POST /api/applications/:id/sources` lists or creates sources; `GET/PATCH/DELETE /api/sources/:id` inspects or changes one; `POST /api/sources/:id/simulate` queues an explicitly simulated test delivery. Creation and changes require workspace ADMIN or OWNER. Source creation returns a unique ingestion URL. `PATCH` accepts `providerSecret`, `destinationUrl`, `status`, and, for CUSTOM, `manualConfig`. The destination must pass the existing public HTTPS SSRF validator; the secret is encrypted and never returned.
