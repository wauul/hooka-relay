...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await workspaceTransaction(app.workspaceId, await userId(), "manage", tx => tx.endpoint.update({ where: { id: endpoint.id }, data: { status: endpointTransition(endpoint.status, "pause") } }));
