...
export async function PATCH(req: Request, { params }: Context) {
  try {
    ...
    return Response.json(
      await workspaceTransaction(id, uid, "manage", (tx) =>
        tx.workspace.update({ where: { id }, data: { name } }),
      ),
    );
  } catch (e) {
    return apiError(e);
...
export async function DELETE(req: Request, { params }: Context) {
  try {
    sameOrigin(req);
    const { id } = await params;
    await workspaceTransaction(id, await userId(), "delete", (tx) =>
      tx.workspace.delete({ where: { id } }),
    );
