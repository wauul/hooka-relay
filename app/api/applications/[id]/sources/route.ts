...
const inputSchema = z.object({ name: z.string().trim().min(1).max(100), provider: z.enum(providerNames), customerId: z.string().min(1).max(100) });
...
export async function POST(req: Request, { params }: Context) {
  try {
    ...
    const source = await workspaceTransaction(app.workspaceId, await userId(), "manage", async tx => {
      ...
      return tx.webhookSource.create({ data: { applicationId: app.id, customerId, name: input.name, provider: input.provider, ingestionToken: randomBytes(32).toString("hex") } });
    });
