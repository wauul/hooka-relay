...
export async function POST(req: Request) {
  try {
    ...
    const app = await workspaceTransaction(workspaceId, uid, "manage", tx => tx.application.create({ data: { name: input.name, workspaceId, customerMode: "ISOLATED", currentApiKey: hashApiKey(plaintext) } }));
