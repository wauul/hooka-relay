...
export async function POST(req: Request) {
  try {
    ...
    const data = z.object({ token: z.string().length(64), password: z.string().min(12).max(72) }).strict().parse(await boundedJson(req));
    const callbackPath = await consumeAuthToken(data.token, "RESET", data.password);
