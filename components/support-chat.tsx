...
export function SupportChat() {
  ...
  return <aside className="support-widget" aria-label="Hooka Relay Support Assistant">
    <section className={`support-panel ${open ? "is-open" : ""}`} aria-hidden={!open} inert={!open} role="dialog" aria-labelledby="support-title" onKeyDown={event => { if(event.key === "Escape") { event.stopPropagation(); close(); } }}>
    ...
    <button ref={launcher} className="btn secondary support-launcher" hidden={open} onClick={() => setOpen(true)} aria-expanded={open} aria-controls="support-title"><MessageCircle size={18} aria-hidden="true" />Hooka Relay support</button>
  </aside>;
}
