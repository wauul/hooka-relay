...
export function EventSchemas({ applicationId, canManage }: { applicationId: string; canManage: boolean }) {
  ...
  return <section className="panel panel-body" style={{ marginTop: 24 }}><h2>Event schemas</h2><p className="muted">Optionally validate payloads for a specific event type. Types without a schema are accepted as before.</p><ErrorBox error={error || failure} />
    ...
    {canManage && <details><summary>Add or replace a schema</summary><form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); try { void save({ eventType: f.get("eventType"), schema: JSON.parse(String(f.get("schema"))) }, "PUT"); } catch { setFailure("Enter valid JSON."); } }} style={{ marginTop: 20 }}>
      <div className="field"><label htmlFor="schema-type">Event type</label><input id="schema-type" name="eventType" placeholder="order.shipped" maxLength={120} required /></div>
