...
export function EndpointSigning({ endpoint, reload }: { endpoint: { id: string; role: string; previousSecretExpiresAt: string | null }; reload: () => Promise<unknown> }) {
  ...
  return <section className="panel panel-body"><h2><T text={"Signing & rotation"} /></h2><p>Deliveries use Standard Webhooks: signed event ID, timestamp, and exact body.</p>
    ...
    {endpoint.role !== "MEMBER" && <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 16 }}><button className="btn secondary" disabled={busy} onClick={() => action("rotate-secret", {}, "POST")}><KeyRound size={16} aria-hidden="true" /><T text={"Rotate signing secret"} /></button></div>}
