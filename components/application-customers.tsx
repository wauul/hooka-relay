...
import { KeyRound, RotateCw, ShieldCheck, ShieldOff, Users, Waypoints } from "lucide-react";
...
export function ApplicationCustomers({ applicationId, canManage }: { applicationId: string; canManage: boolean }) {
  ...
  return <section className="panel panel-body customer-section">
    ...
    {data?.length ? <div className="customer-grid">{data.map(customer => <article className="customer-card" key={customer.id}>
      ...
      <div className="customer-card-actions"><Link className="customer-details-link" href={`/applications/${applicationId}/customers/${customer.id}`}>View customer</Link>{canManage && <div className="customer-portal-actions"><button type="button" className="btn customer-open" disabled={busy} onClick={() => void portal(customer.id, "issue")}>Open portal</button>{customer.portalEnabled && <><button type="button" className="customer-icon-action" aria-label={`Rotate portal link for ${customer.name}`} disabled={busy} onClick={() => void portal(customer.id, "rotate")}><RotateCw size={17} aria-hidden="true" /><span role="tooltip">Rotate & open new link</span></button><button type="button" className="customer-icon-action danger" aria-label={`Revoke portal link for ${customer.name}`} disabled={busy} onClick={() => void portal(customer.id, "revoke")}><ShieldOff size={17} aria-hidden="true" /><span role="tooltip">Revoke link</span></button></>}</div>}</div>
