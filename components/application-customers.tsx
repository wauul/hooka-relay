...
export async function openCustomerPortal(applicationId: string, customerId: string) {
  const tab = window.open("about:blank", "_blank");
  if (!tab) throw new Error("Allow pop-ups to open the customer portal in a new tab.");
  tab.opener = null;
  try {
    const result = await api<{ portalPath: string | null }>(`/api/applications/${applicationId}/customers/${customerId}/portal`, { action: "issue" });
...
export function ApplicationCustomers({ applicationId, canManage }: { applicationId: string; canManage: boolean }) {
  ...
  async function portal(customerId: string, action: "issue" | "rotate" | "revoke") {
    ...
    try {
      if (action === "issue") await openCustomerPortal(applicationId, customerId);
