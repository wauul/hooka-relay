...
This branch requires the offline credential migration and receiver signature update described in [SECURITY.md](SECURITY.md) before production deployment. Request bodies over 256 KiB return 413; JSON depth over 32 returns 400. IP throttling runs before authentication and returns 429 with `Retry-After: 60` and `{ "error": "Too many requests. Try again shortly.", "retryAfter": 60 }`. The existing per-application admission limit is separate.
